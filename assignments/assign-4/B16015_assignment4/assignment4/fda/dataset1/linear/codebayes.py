...
def plot():
	...
	plt.scatter(mainList[2][:,0],mainList[2][:,1],marker='.',color=colors[3])
	plotname = "Class23data.png"
	plt.title("Class 2 and Class 3")
	plt.xlabel('X')
	plt.ylabel('Y')
	class_colours = [colors[5], colors[3]]
	classes = ["Class 2 Data", "Class 3 Data"]
	recs = []
	...
	plt.show()
	plt.scatter(projectedData[2][0],np.zeros(len(projectedData[2][0]),float),marker='.',color=colors[5])
	plt.scatter(projectedData[2][1],np.zeros(len(projectedData[2][1]),float),marker='.',color=colors[3])
	plotname = "Class23reducedata.png"
	plt.title("Class 2 and Class 3")
	plt.xlabel('X')
	plt.ylabel('Y')
	class_colours = [colors[5], colors[3]]
	classes = ["Class2 reduce Data", "Class3 reduce Data"]
	recs = []
	for i in range(0,len(class_colours)):
		recs.append(mpatches.Rectangle((0,0),1,1,fc=class_colours[i]))
	plt.legend(recs, classes, loc='upper right')
