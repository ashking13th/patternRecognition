...
meanVector = []
clusters = np.zeros(
	(numOfClusters, np.size(wholeData, axis=1)), dtype=np.float64)
...
while True:
	a = datetime.now()
	J = assignDataPt()
	print(counter, " : J: ", J, "\t : ", (Jprev-J),
	      " : ", (datetime.now()-loopStarttime))
	# meanVector, " : ",
	# print(counter, " : Cost = ", (Jprev-J))
	counter += 1
	if len(wholeData[0]) < 3:
		gp.plotClustersAndMean(args['output']+str(counter)+"_", wholeData,
		                       numOfClusters, pointsAssignCluster, meanVector, "K-Means")
